...
class ExaminationBadges extends StatelessWidget {
  ...
  int get recommendedIntervalInMonths => categorizedExamination.examination.intervalYears.toInt() * 12;

  int get recommendedIntervalInMonthsMinusTwoMonths => categorizedExamination.examination.intervalYears.toInt() * 12 - 2;
  ...
  BadgeState _getBadgeState(Badge? data, int index){
    if (categorizedExamination.examination.state ==
        ExaminationStatus.CONFIRMED && !isPlannedDate &&
        (isLastConfirmedDateOlderMinusTwoMonths) &&
        index + 1 == data?.level) {
